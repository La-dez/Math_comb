...
#размещения, Идея: как в перестановках, но остановка на глубине k
def generate_arrangements(arr, k, path=[]): 
    if len(path) == k:
        print(path)
        return
    for i in range(len(arr)):
...
#сочетания, Идея: выбираем элементы только из нетронутой части пути, т.е. всегда двигаемся вперёд + глубина
def generate_combinations(arr, k, index=0, path=[]): 
    if len(path) == k:
        print(path)
        return
    for i in range(index, len(arr)):
        #во избежание повторений, комбинации не составляются с буквами, которые уже были в пути
